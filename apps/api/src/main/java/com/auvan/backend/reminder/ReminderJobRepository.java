package com.auvan.backend.reminder;
...
public interface ReminderJobRepository extends JpaRepository<ReminderJob, UUID> {
    ...
    @Query("""
            ...
            WHERE r.status = com.auvan.backend.reminder.ReminderStatus.PENDING
              AND r.runAt <= :now
    ...
    @Modifying
    @Query("""
            UPDATE ReminderJob r SET r.status = com.auvan.backend.reminder.ReminderStatus.CANCELLED
            WHERE r.booking.id = :bookingId
              AND r.status = com.auvan.backend.reminder.ReminderStatus.PENDING
            """)
