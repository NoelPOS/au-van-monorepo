...
public interface ReminderJobRepository extends JpaRepository<ReminderJob, UUID> {
    ...
    @Query("""
            ...
            WHERE r.status = com.auvan.backend.enums.ReminderStatus.PENDING
              AND r.runAt <= :now
    ...
    @Modifying
    @Query("""
            UPDATE ReminderJob r SET r.status = com.auvan.backend.enums.ReminderStatus.CANCELLED
            WHERE r.booking.id = :bookingId
              AND r.status = com.auvan.backend.enums.ReminderStatus.PENDING
            """)
